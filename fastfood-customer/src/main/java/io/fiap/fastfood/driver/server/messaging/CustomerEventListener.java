...
@Component
public class CustomerEventListener implements CommandLineRunner {
    ...
    public CustomerEventListener(@Value("${application.consumer.delay:10000}")
                                 String delay,
                                 @Value("${application.consumer.poolSize:1}")
                                 String poolSize,
                                 CustomerUseCase service) {
        this.service = service;
        boundedElastic = Schedulers.newBoundedElastic(Integer.parseInt(poolSize), 10000,
            "customerListenerPool", 600, true);
